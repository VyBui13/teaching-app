...
export const ToolPropertyBar: React.FC<Props> = ({
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-2 flex flex-wrap items-center justify-between gap-3 z-30 select-none text-slate-800 dark:text-slate-100 shadow-sm transition-colors">
                  ...
                  {isPencilMenuOpen && (
                    ...
                    >
                      <div className="space-y-1.5">
                        <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Bảng màu nét vẽ:</span>
                      ...
                      <div className="space-y-1 pt-1 border-t border-slate-200 dark:border-slate-800">
                        <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 dark:text-slate-400">
                          <span>Cỡ nét vẽ:</span>
                          <span className="text-indigo-600 dark:text-indigo-400">{settings.strokeWidth}px</span>
                        </div>
                        <input
                          type="range"
                          min="1"
                          ...
                          onChange={(e) => onChangeSettings({ strokeWidth: Number(e.target.value) })}
      ...
      {/* Center/Right Group: Contextual Toolbar for Selected Annotation */}
      {selectedAnnotation && (
        <div className="flex items-center gap-2 bg-indigo-50/80 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/30 px-3 py-1 rounded-2xl text-xs font-semibold animate-in fade-in duration-150">
          <span className="text-indigo-600 dark:text-indigo-400 font-bold text-[11px]">
            📌 {selectedAnnotation.type === 'text' ? 'Khung Chữ' : 'Đối tượng đang chọn'}
          </span>

          {/* Color palette swatches for selected annotation */}
          <div className="flex items-center gap-1 pl-1 border-l border-indigo-200 dark:border-indigo-500/30">
          ...
          {/* Font Size select for Text Box */}
          {selectedAnnotation.type === 'text' && (
            <div className="flex items-center gap-1.5 pl-1 border-l border-indigo-200 dark:border-indigo-500/30">
              <span className="text-[11px] text-slate-500 dark:text-slate-400">Cỡ chữ:</span>
              <select
                value={selectedAnnotation.fontSize || 20}
                onChange={(e) => {
                  const size = Number(e.target.value);
                  if (onUpdateSelectedAnnotation && selectedAnnotation) {
                    const updated = AnnotationEntity.create(
                      { ...selectedAnnotation.toJSON(), fontSize: size, updatedAt: Date.now() },
                      selectedAnnotation.id
                    );
                    onUpdateSelectedAnnotation(updated);
                  }
                ...
                className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold px-1.5 py-0.5 rounded-md focus:outline-none cursor-pointer"
              >
              ...
              {onEditSelectedText && (
                <button
                  ...
                  className="px-2 py-0.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-[11px] font-bold flex items-center gap-1 transition shadow-sm"
                >
          ...
          {onDeleteSelectedAnnotation && (
            <button
              ...
              className="px-2 py-0.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 font-bold rounded-lg text-[11px] transition ml-1"
      ...
      {/* Right Group: Text Font Size & Actions */}
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {!selectedAnnotation && settings.activeTool === 'text' && (
          <select
            ...
            className="bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 rounded-xl px-2 py-1 text-xs font-semibold focus:outline-none"
          >
